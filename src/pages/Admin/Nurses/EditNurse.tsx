...
export default function EditNurse() {
  ...
  useEffect(() => {
    if (id) {
      fetchNurse(parseInt(id));
    }
  }, [id]);

  const fetchNurse = async (userId: number) => {
    try {
      setFetching(true);
      ...
      showToast("Failed to load nurse details", "error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ ...errors, submit: err.response?.data?.message || "Failed to update nurse" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in pb-12">
      <button onClick={() => navigate("/admin/nurses")} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors">
        <ArrowLeft size={16} /> Back to Nurses
      </button>

      <div>
        <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Edit Nurse</h1>
        <p className="text-slate-500 mt-1">Update details for this nurse.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="bg-slate-50 p-6 border-t border-slate-100 flex justify-end gap-3">
          <button type="button" onClick={() => navigate("/admin/nurses")} className="px-6 py-2.5 rounded-xl border border-slate-300 text-slate-700 font-semibold hover:bg-slate-100 transition">
            Cancel
          </button>
