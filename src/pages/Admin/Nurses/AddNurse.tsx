...
const genUsername = () => {
  return `nur${String(Math.floor(Math.random() * 1000000)).padStart(6, '0')}`;
};

const genPassword = (username: string) => {
  return `Nur@${username.replace('nur', '')}`;
};
...
export default function AddNurse() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ ...errors, submit: parseBackendError(err, "Failed to create nurse.") });
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center animate-fade-in">
        <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-sm text-center border border-slate-100">
          ...
          <p className="mb-8 text-slate-500">Nurse registered successfully. Provide them with their credentials below.</p>
    ...
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Register nursing staff</p>
        </div>
        <button
          onClick={() => navigate("/admin/nurses")}
      ...
      <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-slate-100">
            <button type="button" onClick={() => navigate("/admin/nurses")} className="rounded-xl border border-slate-200 bg-white px-6 py-3 font-semibold text-slate-600 hover:bg-slate-50 transition">
              Cancel
            </button>
