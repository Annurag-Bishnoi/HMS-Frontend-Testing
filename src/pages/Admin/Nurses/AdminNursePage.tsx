...
import NurseDetailsModal from "../../../components/admin/nurses/NurseDetailsModal";
...
export default function AdminNursePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const staff = all.filter((u: any) =>
        u.roleCode === 'NURSE' || u.role === 'NURSE' ||
        (u.roles || []).some((r: any) => r === 'NURSE' || r?.roleCode === 'NURSE')
      );
      setNurses(staff);
    } catch (err) {
      console.error(err);
  ...
  const handleToggleStatus = async (staff: any) => {
    ...
    try {
      ...
      setNurses(prev => prev.map(s => s.userId === staff.userId ? { ...s, active: !staff.active } : s));
    } catch (err) {
  ...
  const filteredStaff = nurses.filter(staff =>
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Manage Nurses</h1>
          <p className="text-slate-500 mt-1">View and manage all nursing staff.</p>
        </div>
        <button 
          onClick={() => navigate('/admin/nurses/add')}
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <TableEmptyRow colSpan={7} message="No nurses found." />
              ) : (
                currentStaff.map((staff) => (
                  <tr key={staff.userId} className={tableRowClass}>
                    ...
                    <TableActionCell>
                      ...
                      <TableEditButton onClick={() => navigate(`/admin/nurses/edit/${staff.userId}`)} title="Edit Details" />
